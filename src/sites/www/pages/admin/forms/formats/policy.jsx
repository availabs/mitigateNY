...
const Layout = ({children, title, baseUrl}) => {
    ...
    const url = baseUrl //.replace('149258', params.formid)
    return (
            ...
            <div className='flex items-center'>
                ...
                <div className='px-1'>
                    <Link to={`${url}/list/0/10`} className='inline-flex w-36 justify-center rounded-lg cursor-pointer text-sm font-semibold py-1 px-4 bg-blue-600 text-white hover:bg-blue-500 shadow-lg border border-b-4 border-blue-800 hover:border-blue-700 active:border-b-2 active:mb-[2px] active:shadow-none'>
                        Policy Documents Home
                    </Link>
                </div>
                <div className='px-1'>
                    <Link to={`/admin/forms/manage/149258`} className='inline-flex w-36 justify-center rounded-lg cursor-pointer text-sm font-semibold py-1 px-4 bg-blue-600 text-white hover:bg-blue-500 shadow-lg border border-b-4 border-blue-800 hover:border-blue-700 active:border-b-2 active:mb-[2px] active:shadow-none'>
                        Meta
                    </Link>
                </div>
            </div>
...
const TableComp = ({data, columns, ...rest}) => {
    ...
    return <Table data={data} // filter data from fromIndex
                  ...
                  onPageChange={(p) => {
                      ...
                      navigate(`/admin/forms/form/149258/list/${nextFromINdex}/${nextToIndex}`)
                  }}
    />
}
const siteConfig = {
    formatFn: async () => {
        ...
        const config = formConfigs.find(fc => fc.name === 'Policy Documents')?.config;
    ...
    children: [
        {
            type: (props) => <Layout {...props} title={'Policy Documents'} baseUrl={'/admin/forms/form/149258'}/>,
            path: '/*',
            action: 'list',
            filter: {
