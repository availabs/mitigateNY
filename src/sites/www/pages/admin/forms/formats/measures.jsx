...
const Layout = ({children, title, baseUrl}) => {
    ...
    const url = baseUrl //.replace('130882', params.formid)
    return (
            ...
            <div className='flex items-center'>
                ...
                <div className='px-1'>
                    <Link to={`${url}/list/`} className='inline-flex w-36 justify-center rounded-lg cursor-pointer text-sm font-semibold py-1 px-4 bg-blue-600 text-white hover:bg-blue-500 shadow-lg border border-b-4 border-blue-800 hover:border-blue-700 active:border-b-2 active:mb-[2px] active:shadow-none'>
                        Mitigation Measures Home
                    </Link>
                </div>
                <div className='px-1'>
                    <Link to={`/admin/forms/manage/130882`} className='inline-flex w-36 justify-center rounded-lg cursor-pointer text-sm font-semibold py-1 px-4 bg-blue-600 text-white hover:bg-blue-500 shadow-lg border border-b-4 border-blue-800 hover:border-blue-700 active:border-b-2 active:mb-[2px] active:shadow-none'>
                        Meta
                    </Link>
                </div>
            </div>
...
const TableComp = ({data, columns, ...rest}) => {
    ...
    return <Table data={data} // filter data from fromIndex
                  ...
                  onPageChange={(p) => {
                      ...
                      navigate(`/admin/forms/form/130882/list/${nextFromINdex}/${nextToIndex}`)
                  }}
    />
}
const siteConfig = {
    formatFn: async () => {
        ...
        const config = formConfigs.find(fc => fc.name === 'Mitigation Measures')?.config;
    ...
    children: [
        {
            type: (props) => <Layout {...props} title={'Mitigation Measures'} baseUrl={'/admin/forms/form/130882'}/>,
            path: '/*',
            action: 'list',
            filter: {
                ...
                type: props =>
                    <TableComp
                        ...
                        columns={(formid) => [
                            ...
                            // { type: 'link',
                            //     name: '',
                            //     to: '/admin/forms/form/130882/edit/:id',
                            //     text: "edit"
                            // }
                        ]}
                        {...props}
