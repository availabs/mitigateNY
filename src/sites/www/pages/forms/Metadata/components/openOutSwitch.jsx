...
export const IsOpenOutSwitch = ({update, metadata, setMetadata, col, value}) => {
    const onChange = React.useCallback(async e => {
        await editMetadata({update, metadata, setMetadata, col, value: {openOut: e}});
    }, [col, metadata]);
    ...
    return (
        <div className={'block w-full flex mt-1 items-center'}>
            ...
            <div className={'align-bottom p-2 my-1 rounded-md shrink self-center'}>
                ...
                >
                    <span className="sr-only">toggle is open out</span>
                    <span
                        aria-hidden="true"
                        className={
                        `
