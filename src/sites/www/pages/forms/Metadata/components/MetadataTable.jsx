...
import {ManageOptionsSelector} from "./optionsSelector.jsx";
...
export const MetadataTable = ({meta = [], update, colOrigin}) => {
    ...
    return (<div className="overflow-hidden">
        {authLevel > 5 && colOrigin === 'calculated-column' &&
            <AddCalculatedColumn update={update} metadata={metadata} setMetadata={setMetadata}/>}
        <div className={`py-4 sm:py-2 sm:grid sm:${gridCols} sm:gap-4 sm:px-6 border-b-2`}>
            {
                tableCols
                    ...
                    .map((tableCol, i) => (
                        <dd key={i}
                            className="text-sm font-medium text-gray-600 flex justify-between">
                            {tableCol.name}
                            {
                                i === tableCols.length - 1 ?
                                    <i
                                        title={'expand all'}
                                        className={'fa fa-angle-down text-lg px-4'}
                                        onClick={e => setOpen(open === 'all' ? undefined : 'all')}
                                    /> :
                                    null
                            }
                        </dd>
...

                {
                    metadata?.attributes
                        ...
                        .map((col, i) => (
                            <div>
                                <div key={i}
                                     className={`py-4 sm:py-5 sm:grid sm:${gridCols} sm:gap-4 sm:px-6 ml-1 hover:ml-0 border-blue-500 hover:border-l-4`}>
                                    <dt className="text-sm text-gray-900">
                                        <div className={'flex flex-row justify-between group'}>
                                            {editing === `${col.name}-columnName` ? <div className='pt-3 pr-8'>
                                                ...
                                                <div className='truncate pt-3 pr-8'>{get(col, 'name') || 'No Name'}</div>}
                                        ...
                                        <div className={'flex flex-row justify-between group'}>
                                            {editing === `${col.name}-displayName` ? <div className='pt-3 pr-8'>
                                                ...
                                                className='pt-3 pr-8 font-bold'>{get(col, 'display_name') || 'No Display Name'}</div>}
                                    ...
                                    {authLevel > 5 &&
                                        <dd className="mt-1 text-sm text-gray-900 sm:mt-0 items-center">
                                            {
                                    ...
                                    {authLevel > 5 &&
                                        <dd className="mt-1 text-sm text-gray-900 sm:mt-0 flex flex-row">
                                            ...
                                            <i
                                                title={'expand'}
                                                onClick={e => setOpen(open === col.name ? undefined : col.name)}
                                                className={'fa fa-angle-down text-lg hover:text-blue-500 px-4 py-2 transition ease-in'}
                                            />
                                        </dd>
                                    }

                                    <div className={[col.name, 'all'].includes(open) ? 'col-span-5 bg-blue-50 py-4 sm:px-6' : 'hidden'}>
                                        {
                                            authLevel > 5 ?
                                                <IsOpenOutSwitch
                                                    update={update}
                                        ...
                                        {
                                            ['select', 'multiselect', 'radio'].includes(col.type) &&
                                            <ManageOptionsSelector
                                                update={update}
                                                metadata={metadata}
                                                setMetadata={setMetadata}
                                                col={col.name}
                                                startValue={get(col, 'options') || 'No Options Available'}
                                                attr={'options'}
                                                setEditing={setEditing}
                                                cancel={() => setEditing(null)}
                                            />
