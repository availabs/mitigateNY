...
import { RenderVersions, getType } from "../utils/macros.jsx";
const CallServer = async ({rtPfx, baseUrl, source, newVersion, navigate, viewIHP, viewState, viewCounty, user}) => {
    ...
    const body = JSON.stringify({
        table_name: getType(source, 'ihp_valid_registrations_v2_enhanced'),
...
const Create = ({ source, newVersion, baseUrl, context }) => {
    ...
    React.useEffect(() => {
        async function fetchData() {
            await getSrcViews({rtPfx, falcor, pgEnv, setVersions: setVersionsIHP,  type: 'individuals_and_households_program_valid_registrations_v2'});
