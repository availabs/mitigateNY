...
const CallServer = async ({rtPfx, baseUrl, source, newVersion, navigate, viewPa, viewDDS, user}) => {
    ...
    const url = `${rtPfx}/hazard_mitigation/enhance-pa-funded-projects-v2`
    const body = JSON.stringify({
        table_name: getType(source, 'pa_funded_projects_enhanced'),
...
const Create = ({ source, newVersion, baseUrl }) => {
    ...
    React.useEffect(() => {
        async function fetchData() {
            await getSrcViews({rtPfx, falcor, pgEnv, setVersions: setVersionsPa,  type: 'public_assistance_funded_projects_details_v2'});
